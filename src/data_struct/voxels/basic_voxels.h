...
template <typename T>
bool BasicVoxels<T>::ConvertPositionTo3DofIndices(const Vec3 &position, typename Voxels<T>::Index &voxel_indices) const {
...
template <typename T>
bool BasicVoxels<T>::ConvertPositionTo3DofIndices(const Vec3 &position, typename Voxels<T>::Index &voxel_indices, typename Voxels<T>::Index &map_indices) const {
    for (uint32_t i = 0; i < voxel_indices.size(); ++i) {
        voxel_indices[i] = static_cast<int32_t>(std::floor(position[i] / this->options().kStep[i])) + this->half_voxel_length()[i];
...
template <typename T>
bool BasicVoxels<T>::TryToOccupy(const typename Voxels<T>::Index &voxel_indices, const T &value) {
...
template <typename T>
bool BasicVoxels<T>::IsOccupied(const typename Voxels<T>::Index &voxel_indices, const T &value) {
    const uint32_t index = this->GetBufferIndex(voxel_indices[0], voxel_indices[1], voxel_indices[2]);
    auto &v = GetVoxel(index);
    return v == value;
}
...
template <typename T>
void BasicVoxels<T>::ClearVoxel(const typename Voxels<T>::Index &voxel_indices) {
    const uint32_t index = this->GetBufferIndex(voxel_indices[0], voxel_indices[1], voxel_indices[2]);
