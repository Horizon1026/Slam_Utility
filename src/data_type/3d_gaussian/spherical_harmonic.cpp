...
namespace {
    ...
    constexpr std::array<float, 5> kC2 =
        std::array<float, 5> {1.0925484305920792, -1.0925484305920792, 0.31539156525252005, -1.0925484305920792, 0.5462742152960396};
    constexpr std::array<float, 7> kC3 = std::array<float, 7> {-0.5900435899266435, 2.890611442640554, -0.4570457994644658, 0.3731763325901154,
...
float SphericalHarmonicFunc<2>::GetColorInFloat(const Vec3 &direction) const {
    ...
    const float yz = y * z;
    const float xz = x * z;
    color += kC2[0] * xy * coeff_[4] + kC2[1] * yz * coeff_[5] + kC2[2] * (2.0f * zz - xx - yy) * coeff_[6] + kC2[3] * xz * coeff_[7] +
...
float SphericalHarmonicFunc<3>::GetColorInFloat(const Vec3 &direction) const {
    ...
    // Order 3.
    color += kC3[0] * y * (3.0f * xx - yy) * coeff_[9] + kC3[1] * xy * z * coeff_[10] + kC3[2] * y * (4.0f * zz - xx - yy) * coeff_[11] +
             kC3[3] * z * (2.0f * zz - 3.0f * xx - 3.0f * yy) * coeff_[12] + kC3[4] * x * (4.0f * zz - xx - yy) * coeff_[13] +
             kC3[5] * z * (xx - yy) * coeff_[14] + kC3[6] * x * (xx - 3.0f * yy) * coeff_[15];
    return color;
}
