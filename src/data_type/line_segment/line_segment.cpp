...
Vec3 LinePlucker3D::ProjectToNormalPlane() const { return normal_vector(); }

Vec3 LinePlucker3D::ProjectToImagePlane(const float fx, const float fy, const float cx, const float cy) const {
    Mat3 K = Mat3::Zero();
    K << fy, 0, 0, 0, fx, 0, -fy * cx, -fx * cy, fx * fy;
    return K * normal_vector();
}

template <>
Mat6x4 LinePlucker3D::LinearizeTo4Dof<true>() const {
...
template <>
void LinePlucker3D::UpdateParameters<true>(const Vec4 &delta_param) {
...
template <>
Mat6x4 LinePlucker3D::LinearizeTo4Dof<false>() const {
    const Mat3 U = matrix_U();
    const Mat3x2 W = matrix_W();
    const Vec3 u1 = U.col(0);
    const Vec3 u2 = U.col(1);
...
template <>
void LinePlucker3D::UpdateParameters<false>(const Vec4 &delta_param) {
    // Update part of rotation.
    const Vec3 omega = delta_param.head<3>();
    Mat3 R = Mat3(Utility::Exponent(omega));
